...
- The base (vanilla) model which demonstrated the best performance, underwent fine-tuning. We utilized the Montreal Forced Aligner (MFA) to generate 10-second clips from the 10 videos that depicted medical consultations in Spanish from Latin America. Data augmentation was employed to enhance the dataset. The augmented data of 10-second clips were utilized in the fine-tuning process, which comprised training, validation, and testing phases in a 70-15-15 ratio. The original human-made transcription served as the ground truth. Finally, the fine-tuned model was applied to the full videos, similar to the base models.
